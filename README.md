...
* Komutu girdikten sonra terminal ekranında *"We're unable to detect target browsers. Would you like to add the defaults to your package.json? (Y/n)"* şeklinde soru çıkarsa klavyeden **y** tuşuna basıp Enter'a basın.<br>*(Açıklama: Bu soru, React'in yazdığımız modern kodları tüm tarayıcılarda (Chrome, Safari, Edge vb.) kusursuz ve uyumlu çalıştırabilmesi için gereken varsayılan altyapı ayarlarını (browserslist) projeye eklemek istemesinden kaynaklanır. Onayladığınızda sistem eksik ayarları otomatik tamamlar).*
